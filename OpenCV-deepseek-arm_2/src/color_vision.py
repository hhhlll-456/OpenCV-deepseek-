...
────────────────── --suction: 桌面 4 码 + 吸盘第 5 码 ──────────────────
    --suction    再认一个码: 吸盘底座上那个（内容在 data/suction_qr.json）。
                 ★ 桌面 4 码缺一个，方块坐标**根本算不出来**、会直接报错；
                   吸盘码缺了却什么都不影响 —— 一声不吭。所以它必须被单独报出来，
                   否则会一路「看起来正常」到做动态基座定位时才发现从没解出来过。
                 ★ 它贴在末端法兰上、比纸面高，整帧检测**定位不到**它
                   （实测原因见 qr_vision.detect_sweep），所以这条路径要多跑
                   一遍滑窗补扫，慢 0.1~0.2 秒。
                 ★ 报出来的「纸面投影」**没做视差校正**（缺相机高度和内参），
                   只是中间量，**不写进 world_state** —— 那会被当成可信坐标用。
...
from paths import (COLOR_DEBUG_PNG, PAPER_JSON, SUCTION_QR_JSON,   # noqa: E402
                   WORLD_CODES_KEY, WORLD_NEAR_CODES, WORLD_NEAR_KEY,
                   WORLD_NEAR_P1, WORLD_NEAR_P2, WORLD_NEAR_P3, WORLD_NEAR_P4,
...
from qr_vision import (DATA_MODULES, MODULES_TOTAL,               # noqa: E402
                       detect as qr_detect, detect_sweep,
                       load_paper_layout, load_suction_code,
                       px_per_mm, px_per_module, quad_center)
...
def nearest_code_in_paper(H: np.ndarray, layout, px: float, py: float) -> str | None:
    """
    ★ 「这个像素点离纸面上哪个角码最近」—— 返回码名（"P1".."P4"），判不出来返回 None。

    ★ 判的是**相机看见这一刻**这块方块离哪个角最近 —— 结果盖进 near 戳，用来选
      XY 补偿表里那个角（main.GRASP_OFFSET_MM），而 XY 恰恰只在"相机刚给的那一次
      抓取"补，所以这个戳只在那一抓之前有意义。想判"它**此刻**在哪个角附近"
      （高度补偿要的是这个）**不能**用这个戳，得拿 WORLD_CODES_KEY 里那四个码的
      位置现算（见 main.nearest_code_of_xy）—— 两者可能落在两个不同的角上。
    ★ 为什么拿**纸自己的坐标系**判，而不是拿机械臂坐标: 用户报的那套偏差
      （P1/P3 那一带偏右下…）是**纸面上**的位置。四个码的纸面坐标钉死在标定纸
      定义里（P1≈(29.6, 52.5)…），纸被挪了、机位被撞了，它都不变；而机械臂坐标
      是跟着机位一起变的，拿它划区，下次重架摄像头就得重新量一遍边界。
      判据跟纸走，永远不用重标。
    ★ 判不出来（少二维码拟合不出映射、或透视退化）返回 None —— 调用方据此
      **不写** near 键，也就不会补偿。宁可少补 5mm，也不赌错方向白偏。
    """
    try:
        x, y = paper_xy(H, px, py)
    except VisionError:
        return None
    centers = layout.mm_for("center")
    best, best_d = None, None
    for code in layout.codes:
        cx, cy = centers[code]
        d = float(np.hypot(x - float(cx), y - float(cy)))
...
def build_world_state(cubes: dict[str, dict], mode: str, decoded, layout,
    ...
    返回 {颜色: {"x":.., "y":.., "z_level":.., "src":"camera"[, "near":"P1".."P4"]}}；
    缺方块会抛 VisionError。
    ★ robot 模式还会多一条**保留键** WORLD_CODES_KEY（四个角码各自在机械臂坐标系的
      哪儿）—— 它**不是**一条方块记录，遍历记忆库的地方必须跳过（见 paths 里那段约定）。

    ★ near 是「**相机看见这一刻**这块方块离哪个角最近」，用来选 XY 补偿表里那个角
      （见 main.GRASP_OFFSET_MM 与 paths.WORLD_NEAR_KEY 的说明）。
    ...
    # ★ 「像素→纸面」矩阵: 两个模式都要用它判最近角码，所以在这儿算**一次**。
    #   paper 模式没有它整体就没意义 —— 原样抛出（下面分支里 re-raise 同一个异常）。
    #   robot 模式没有它只是判不出角码 —— 容忍，不写 near 键。
    ...
    def _entry(color: str, x: float, y: float) -> dict:
        ...
        ★ 每条记录都盖两个戳（都是主程序内部用的，不是给模型的字段）:
          · WORLD_SRC_KEY = "camera" —— 「这条 x/y 是相机给的，机械臂还没碰过」。
            这是 XY 补偿那一档的**闸门**: 只有带这个戳的抓取才补（见 main.GRASP_OFFSET_MM）。
          · WORLD_NEAR_KEY —— **相机看见这一刻**这块方块离哪个角最近，用来选补偿表里
            那个角；判不出来就不写这个键（不猜）。
        ★ 执行完 refresh_world_state 会把这条**整条重写**成 {x, y, z_level} ——
        **两个戳一起没**（位置已经换成机械臂确认过的，那几毫米相机偏差根本不存在了，
        再补反而会推偏）。两个戳都只说"这条坐标的来历"，
        **不参与坐标计算** —— print_table / z_level / 各类判据都只认 x/y/z_level。
        ...
        if H_paper is not None:
            near = nearest_code_in_paper(H_paper, layout,
                                         cubes[color]["px"], cubes[color]["py"])
            if near is not None:
    ...
    if mode == "paper":
        ...
        for color in CUBE_COLORS:
            ...
            state[color] = _entry(color, x, y)
        # ★ 顺手把**四个角码自己**此刻的机械臂坐标也存下来（保留键，见 paths.WORLD_CODES_KEY）。
        #   为什么要: 方块那条记录只说了"我的**老家**在哪个角"（near 戳，钉死不动的）。
        #   机械臂把方块搬走之后，主程序要回答「它**现在**离哪个角最近」（高度那一档
        #   按这个算），就得有码的位置现算 —— 而主程序跑不了视觉（cv2）。
        #   只存这一帧真解出来的码: 缺码时 near 本来就判不出来，也就无从谈起判"现在"。
        #   ★ 纸面模式不写这个键: 那边的坐标是纸面 mm，跟方块记录不是一套坐标系，
        #     混进去就是"看着像能用、其实差一个变换"（比没有危险）。
        codes = {}
        for code in layout.codes:
            if code in decoded:
                cx, cy = quad_center(decoded[code])
                x, y = px_to_robot(M, cx, cy)
                codes[code] = [round(x, 2), round(y, 2)]
        if codes:
            state[WORLD_CODES_KEY] = codes
    return state
...
def reset_levels_if_flat(state: dict) -> str | None:
    ...
    if any(c not in state for c in CUBE_COLORS):
        return None
    # ★ 一律按 CUBE_COLORS 走，**不** list(state): 记忆库里除了四条方块记录，
    #   还有带 "_" 的保留键（_codes = 四个角码在哪儿，见 paths.WORLD_CODES_KEY），
    #   它里面没有 "x"/"z_level" —— 当成方块用就是 KeyError。
    ...
    if worst < FLAT_MIN_GAP_MM:
        return None
    dropped = {c: int(state[c]["z_level"]) for c in CUBE_COLORS
               if state[c]["z_level"]}
    if not dropped:
        return None
...
def sanity_check(state: dict, mode: str, layout) -> list[str]:
    ...
    # ★ 按 CUBE_COLORS 挑，**不** list(state): 记忆库还有带 "_" 的保留键
    #   （_codes，见 paths.WORLD_CODES_KEY），它不是方块记录。
    names = [c for c in CUBE_COLORS if c in state]
...
def analyze(frame: np.ndarray, mode: str, layout, robot=None,
            prior=None, suction: str | None = None,
            ) -> tuple[dict, dict, float | None, dict]:
    """
    ...
    ★ suction=吸盘码内容（None = 这次不找它）。给了就走 detect_sweep:
      整帧检测**定位不到**吸盘码（实测，见 qr_vision.detect_sweep 的说明），
      要它必须补一遍滑窗。代价是这一帧慢 120~200ms；整帧就全解出时不补扫。
      解出来的吸盘码只是**多一条 decoded 记录**，不进 world_state
      —— build_world_state 只遍历 CUBE_COLORS，认不认它都不影响方块坐标。
    """
    detector = cv2.QRCodeDetector()
    if suction:
        decoded, located = detect_sweep(frame, detector,
                                        want=list(layout.codes) + [suction])
    else:
        decoded, located = qr_detect(frame, detector)
    ppm = None
...
def print_code_report(decoded: dict, layout, suction: str) -> None:
    """
    报「这一帧解出了哪几个二维码」，吸盘码另算一栏。**只报，不动世界状态。**

    ★ 为什么要单独报这件事: 方块坐标表（print_table）里**看不到码**。
      四码缺一个时 build_world_state 直接报错、反而显眼；可吸盘码缺了不影响
      任何方块坐标 —— 一路静悄悄，直到做动态基座定位时才发现它从来没解出来过。
      所以 --suction 必须把「5 个码各自解出没有」摆到明面上。

    ★ 为什么不写进 world_state.json: 见下面那段「视差」说明。吸盘码直接投到
      纸面 = **把高处的东西当成贴着纸面**，结果系统性偏外。写进去就等着哪天
      main.py 真拿它去驱机械臂 —— 那比不写危险得多（错了还不报错）。
    """
    hits = [c for c in layout.codes if c in decoded]
    marks = "  ".join(("✅ " if c in decoded else "❌ ") + c
                      for c in list(layout.codes) + [suction])
    n_ok = len(hits) + (1 if suction in decoded else 0)
    n_all = len(layout.codes) + 1
    print(f"\n  ── 二维码 {n_ok}/{n_all} ──\n   {marks}")

    if suction not in decoded:
        print(f"\n  ✗ 吸盘码没解出来。桌面 4 码 {len(hits)}/{len(layout.codes)}。")
        print("    · 它贴在末端法兰上，**和纸面不在同一个焦平面**，自动对焦在两者"
              "之间来回找 ——\n      同一个机位实测能 0/30 也能 25/25，别拿一次结果"
              "下结论。")
        print("    · 先用 tools/test_camera_qr.py --shot 把机位调到它稳定解得出来"
              "（那条路逐帧补扫，比这里单帧可靠），再回来跑这条命令。")
        print("    · 机身挡住码/码贴歪/白边被法兰压掉，也都会这样。")
        return

    q = decoded[suction]
    cx, cy = quad_center(q)
    per_mod = px_per_module(q)
    ref = [px_per_module(decoded[c]) for c in hits]
    print(f"\n  吸盘码 {suction!r}  像素中心 ({cx:.1f}, {cy:.1f})"
          f"   {per_mod:.2f} px/模块")
    if ref:
        print(f"    桌面 4 码最小 {min(ref):.2f} px/模块 —— 吸盘码比这个低就是印小了"
              f"（<{qv.OK_PX_PER_MODULE} 开始丢码，>{qv.GOOD_PX_PER_MODULE} 才稳）")

    # 拿四个纸面码拟合出的「像素→纸面」矩阵，把吸盘码中心投过去。
    try:
        H = px_to_paper_map(decoded, layout)
        sx, sy = paper_xy(H, cx, cy)
    except VisionError as e:
        print(f"    （投不到纸面: {e}）")
        return
    pw, ph = layout.paper_mm
    print(f"  纸面投影 ({sx:.1f}, {sy:.1f}) mm   ← **未做视差校正**")
    if not (0 <= sx <= pw and 0 <= sy <= ph):
        print(f"    ⚠ 跑到纸外了（纸 {pw:.0f}x{ph:.0f}）—— 机械臂这时多半不在纸"
              f"正上方，这个投影没意义")
    print("    ★ 吸嘴比纸面高 h，同样的像素点落到纸面会**往外偏**（离光心越远偏得"
          "越多）。\n      真实落点 = 光心 + (投影 − 光心) × (Z−h)/Z，需要相机高度 Z 和"
          "内参（光心）。\n      这两样现在都没有，所以这个数**只是中间量，别拿它驱机械臂**。")
    print("    ★ 也**没有**写进 world_state.json —— 写进去会被当成可信坐标用。")
...
def _selftest_paper_mm(layout, syn, check) -> None:
    ...
    # ★ 每条都得带两个戳: 来源戳（WORLD_SRC_KEY）+ 最近角码戳（WORLD_NEAR_KEY）。
    #   主程序靠 near 决定 XY 要补多少（main.GRASP_OFFSET_MM）。戳丢了不会报错，
    #   只会让补偿**永远不生效**（用户看到的现象是"改了跟没改一样"），
    #   所以形状自检必须把它一起钉住。
    check("paper 模式产出的 world_state 形状对"
          "（四个颜色 + x/y/z_level + 来源戳 + 最近角码戳）",
          sorted(st) == sorted(CUBE_COLORS)
          and all(set(v) == {"x", "y", "z_level", WORLD_SRC_KEY, WORLD_NEAR_KEY}
                  for v in st.values())
          and all(v[WORLD_SRC_KEY] == WORLD_SRC_CAMERA for v in st.values()))
    # ★ 纸面模式**不许**带保留键 _codes: 那边的是纸面 mm，和方块记录不是一套坐标系。
    #   混进去不会当场报错，只会让主程序拿纸面 mm 去跟机械臂 mm 比距离 ——
    #   判出来的"最近角码"看着像回事，其实整个是错的。
    check("纸面模式不带保留键 _codes（那是机械臂坐标系的东西）",
          WORLD_CODES_KEY not in st,
          str(sorted(st)))

    # ★ 最近角码戳必须按**纸面距离**判，四个角别搞混: 合成图里四个方块画在
    #   red(85,95) / yellow(205,95) / green(205,145) / blue(85,145)，四个码在
    #   P1(29.55,52.5) / P2(242.55,52.5) / P3(28.45,183.5) / P4(242.45,183.5)。
    #   最接近的是 red→P1、yellow→P2、green→P4、blue→P3（四个角各中一个）。
    #   判错了不会报错，只会让那 5mm 补到别的角上去。
    nears = {c: st[c].get(WORLD_NEAR_KEY) for c in CUBE_COLORS}
    check("最近角码戳按纸面距离判对（红→P1、黄→P2、绿→P4、蓝→P3）",
          nears == {"red": WORLD_NEAR_P1, "yellow": WORLD_NEAR_P2,
                    "green": WORLD_NEAR_P4, "blue": WORLD_NEAR_P3},
          str(nears))

    # ★ 常量表里的四个码名必须和标定纸 json 里那四个**一字不差**。
    #   main 的补偿表拿这些常量当键（main 不能 import 本文件 → cv2），名字一改，
    #   补偿就查不到键 → 不补 → 每次都偏那几个毫米，还看不出哪儿错了。这条是那段
    #   复制关系唯一的钉死处。合成图里红/黄/绿/蓝分别离四个码最近，所以
    #   `nears` 一套值顺带把"哪个名字对应哪个角"也验了（上面那条 check）。
    check(f"paths 里的四个角码名和标定纸 json 一致 {list(WORLD_NEAR_CODES)}",
          set(WORLD_NEAR_CODES) == set(layout.codes)
          and tuple(WORLD_NEAR_CODES) == tuple(layout.codes),
          f"json 里是 {list(layout.codes)}")

    # ★ 判不出来时**不许写 near**（少一个二维码 → 拟合不出像素↔纸面）。
    #   ★ 这条是「宁可少补 5mm，也不赌错方向」的守门测试: robot 模式少了码，
    #     整帧照常出坐标（源戳在），但角码戳必须缺席。
    ...
    # ★ 只挑四条方块记录来看形状: 记忆库里还多一条带 "_" 的保留键
    #   （_codes，见 paths.WORLD_CODES_KEY），它不是方块记录。
    recs2 = {c: st2[c] for c in CUBE_COLORS}
    check("少一个二维码 → 照样出坐标，但角码戳缺席（判不出来就不猜）",
          all(WORLD_SRC_KEY in v for v in recs2.values())
          and all(WORLD_NEAR_KEY not in v for v in recs2.values())
          # 保留键里也就只剩真解出来的那三个码 —— 缺的那个不许被编出来。
          and set(st2.get(WORLD_CODES_KEY) or {}) == set(layout.codes) - {layout.codes[0]},
          str({c: sorted(v) for c, v in st2.items()}))

    # ★★ robot 模式**必须**把四个角码此刻的机械臂坐标带上（保留键 _codes）。
    #   为什么: 方块被机械臂搬走之后，主程序要判「这块**现在**离哪个角最近」
    #   （高度那一档按它算，见 main.GRASP_OFFSET_Z_MM）—— 而主程序跑不了视觉。
    #   缺了不会报错，只会让那 3mm **静默不生效**（用户看到的是"改了跟没改一样"）；
    #   记成方块的中心（而不是角码的中心）则会判出一整套错的"最近角"。
    stc = build_world_state(syn["cubes"], "robot", syn["decoded"], layout,
                            robot=(np.eye(3, dtype=np.float64),
                                   lambda m, px, py: (px, py)), prior={})
    codes = stc.get(WORLD_CODES_KEY)
    check("robot 模式带保留键 _codes（四个角码的机械臂坐标，名字齐全）",
          isinstance(codes, dict) and set(codes) == set(layout.codes)
          and all(isinstance(v, list) and len(v) == 2 for v in codes.values()),
          str(codes))
    # 这里 px_to_robot 是恒等映射（"机械臂坐标"就取像素），所以 _codes 的值必须
    # 正好等于四个**码**的像素中心 —— 拿方块中心填进去就会被这条抓住。
    want_codes = {c: [round(float(v), 2) for v in quad_center(syn["decoded"][c])]
                  for c in layout.codes}
    check("_codes 记的是**角码自己**的中心，不是方块的",
          codes == want_codes,
          f"_codes={codes}\n      应该是={want_codes}")

    # ★ 「判不出来就不猜」还有**另一条**路: 四个码都在、映射也拟合得出来，但
    #   "最近角码"那一步自己说不知道（透视退化 → paper_xy 抛错 → 返回 None）。
    #   上面那条走的是"H_paper 压根没拟合出来"，**到不了** _entry 里的那个兜底，
    #   所以光有它，"兜底写成了随便塞一个码"这种改法照样能全绿（实测: 把兜底换成
    #   `near or layout.codes[0]` 时上面那条一点反应都没有）。这条专门验那个兜底:
    #   把 nearest_code_in_paper 临时换成"永远不知道"，喂进去的必须是**不带戳**的坐标。
    _real_nearest = nearest_code_in_paper
    try:
        globals()["nearest_code_in_paper"] = lambda *a, **k: None
        st3 = build_world_state(syn["cubes"], "paper", syn["decoded"], layout,
                               prior={})
    finally:
        globals()["nearest_code_in_paper"] = _real_nearest
    check("最近角码那一步说「不知道」时 → 不写戳（不是随手塞一个码）",
          all(WORLD_NEAR_KEY not in v for v in st3.values())
          and all(WORLD_SRC_KEY in v for v in st3.values()),
          str({c: sorted(v) for c, v in st3.items()}))
...
def main() -> int:
    ...
    ap.add_argument("--suction", action="store_true",
                    help=f"★ 一并认吸盘上的第 5 个码（内容读 {SUCTION_QR_JSON.name}），"
                         f"报出 5 个码各自解出没有 + 吸盘位置；比不带慢 0.1~0.2 秒")
    ...
    # ★ 读不到吸盘码内容（没生成过 json）**不拦路**: 方块坐标一点都不依赖它，
    #   为一个附加项把主流程拦下，等于把「认方块」和「贴没贴吸盘码」绑在一起。
    suction = load_suction_code() if args.suction else None
    if args.suction and suction is None:
        print(f"  ⚠ 读不到 {SUCTION_QR_JSON} —— 本次只认桌面 4 码。\n"
    ...
    try:
        ...
        state, cubes, ppm, decoded = analyze(frame, mode, layout, suction=suction)
    except VisionError as e:
        print(f"\n✗ {e}")
        return 1
    ...
    if suction:
        print_code_report(decoded, layout, suction)
