...
from paths import WORLD_CODES_KEY, WORLD_NEAR_KEY, WORLD_SRC_KEY  # noqa: E402

# ★ 记忆库里那些**主程序内部**的戳（每条方块记录**里面**的键），发给模型之前一律剥掉
#   （见 build_messages）。它们不属于输出协议（grasp/place 只有 x/y/z_level），
#   发过去只会让模型多一个要照抄的字段（flash 实测爱照抄输入）。加新戳时**记得加到这里**。
INTERNAL_STATE_KEYS = (WORLD_SRC_KEY, WORLD_NEAR_KEY)

# ★ 记忆库里那些**不是方块记录**的保留键（顶层），发给模型之前整个**整条**丢掉。
#   判据是键名以 "_" 开头（约定见 paths.WORLD_CODES_KEY）—— 不硬写 "_codes" 这一个
#   字面量，以后再加别的保留键不用回来改这里，也就不会漏。
#   ★ 为什么必须丢: 它们长的不一样（_codes 里是码的坐标，没有 "x"/"y"），
#     照抄进 prompt 就是给模型喂垃圾；name_map 更糟 —— 会把 "_codes" 当成一个
#     「方块英文名」列进对照表（"→ _codes"），模型很可能真去发起 obj="_codes" 的动作。


def _is_cube_record(key) -> bool:
...
def build_messages(world_state: dict, instruction: str) -> list[dict]:
    ...
    """
    # ★ 记忆库里每条可能带内部戳 "src"/"near"（x/y 是相机拍的还是机械臂确认的、
    #   离哪个角码最近，见 paths.WORLD_SRC_KEY / WORLD_NEAR_KEY）—— 那是**主程序
    #   内部的账**，不是给模型的字段: 输出协议里 grasp/place 只有 x/y/z_level，
    #   把它们一起发过去就是让模型多几个要照抄的字段（flash 实测爱照抄输入），
    #   纯属给它添乱。剥掉再发（列表见 INTERNAL_STATE_KEYS）。
    # ★ 顶层还有**不是方块记录**的保留键（"_codes" 这类，见 _is_cube_record）——
    #   那些整条**留下不发**，不能进 plain、更不能进 name_map。
    cubes = {k: v for k, v in world_state.items() if _is_cube_record(k)}
    ...
    name_map = "\n".join(f"  {COLOR_CN.get(k, k)} → {k}" for k in cubes)
...
def cmd_selftest() -> int:
    ...
    def t_prompt():
        ...
        # ★ 记忆库里的内部戳（paths.WORLD_SRC_KEY / WORLD_NEAR_KEY）是主程序内部
        ...
        s3 = build_messages({"purple": {"x": 7.0, "y": 9.0, "z_level": 1,
                                        ...
                                        WORLD_NEAR_KEY: "P1"}}, "测试")[0]["content"]
        for k in INTERNAL_STATE_KEYS:
            assert k not in s3, f"内部戳 {k!r} 漏进 prompt 了"
        assert '"x": 7.0' in s3 and '"y": 9.0' in s3, "剥戳时把坐标也剥掉了"

        # ★ 记忆库顶层那些**不是方块记录**的保留键（paths.WORLD_CODES_KEY = "_codes"，
        #   里面是四个角码的机械臂坐标）**整条都不许发** —— 它没有 x/y，照抄进 prompt
        #   就是喂垃圾；更糟的是 name_map 会把 "_codes" 当成一个「方块英文名」列进
        #   对照表，模型很可能真去发起 obj="_codes" 的动作。
        #   ★ 两条都要钉: (1) 用**真的那个键名**（从 paths 取，改名字这里跟着走）；
        #   (2) 用**约定本身**（随便一个下划线开头的键都得挡掉），以后再加保留键
        #      忘了改这里，第 (2) 条照样能抓到。
        s4 = build_messages({"purple": {"x": 7.0, "y": 9.0, "z_level": 0},
                             WORLD_CODES_KEY: {"P1": [100.0, 100.0],
                                               "P2": [400.0, 100.0]},
                             "_whatever": [1, 2, 3]}, "测试")[0]["content"]
        assert WORLD_CODES_KEY not in s4, \
            f"保留键 {WORLD_CODES_KEY!r} 漏进 prompt 了（模型会把它当成一个方块名）"
        assert "_whatever" not in s4, "下划线开头的保留键没被挡住（约定破了）"
        assert "→ purple" in s4 and '"x": 7.0' in s4, "挡保留键时把真方块也挡掉了"
