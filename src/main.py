...
from paths import (PLAN_JSON, ROOT, WORLD_HALF_KEY,     # noqa: E402
...
# ★ 「相机刚拍完、机械臂还没碰过这块方块」的**第一次**抓取，吸盘点在检测位置上
#   再偏这么多（dx, dy），单位 mm。**按方块在纸的哪半边分别给**
#   （键 = paths 里的 WORLD_HALF_LEFT / WORLD_HALF_RIGHT）。
#
#   为什么有这一条（2026-09-18 用户实测）: 方块坐标刚被相机刷新过时，**第一抓**
#   落地总跟检测位置对不上；同一块方块再抓第二次就准了。所以只该补第一次，不能
#   一路补下去 —— 一路补就是把这点误差**永久**写进每一条指令。
#
#   ★ 两半的偏法不一样，所以两个方向各给各的（用户分别报的）:
#     · 左半边（P1/P3 那侧）: 「吸盘偏左」→ 往**机械臂右边**挪，dy = −5.0。
#       右 = Y− 这条口径见 deepseek_brain.SIDE_OFFSET_MM 的注释; 写成 +5.0
#       就是反方向，越补越偏。
#     · 右半边（P2/P4 那侧）: 「吸盘的 X 比实测值**大** 5mm」→ dx = +5.0。
#   dx/dy 说的都是**机械臂坐标**，正负直接照机械臂来，不折算画面方向。
#
#   ★ 判据是「这条坐标谁给的 + 它在纸的哪半边」，不是"第几个动作": 这块方块一旦被
#     抓过，refresh_world_state 会把它那条**整条重写**成机械臂确认过的坐标
#     （两个戳都没了），下一次抓它就按真实值走。详见 plan_action ⑥。
#   ★ 「哪半边」由 color_vision 在**纸自己的坐标系**里算好、随记录写进 world_state
#     的 half 戳（paths.WORLD_HALF_KEY），不是在这儿拿机械臂坐标猜 —— 机位一变，
#     机械臂坐标的"左右分界线"就跟着变，纸面的分界线却是钉死的。
#   ★★ half 判不出来（少二维码）时**不补**: 下面是按 half **下标取值**，取不到就是
#     None。宁可少补 5mm，也不赌错方向白偏。
FIRST_GRASP_OFFSET_MM = {
    WORLD_HALF_LEFT:  (0.0, -5.0),   # 左半边: 吸盘偏左 → 往右挪（Y − 5）
    WORLD_HALF_RIGHT: (+5.0, 0.0),   # 右半边: X 比实测值大 5mm
}
...
def plan_action(action: dict, table_z: float, obj_h: float, net_h: float | None,
    ...
    # ⑥ 「相机刚拍完、机械臂还没碰过这块方块」的**第一次**抓取: 吸盘点按
    #    FIRST_GRASP_OFFSET_MM 挪一下，补偿实测的「第一抓跟检测位置对不上」。
    #    ★ 只改 gx/gy 这**两个局部变量**，绝不写回 world / action —— 「不影响后面对
    #      该物体实际位置的计算」全靠这一条: 记忆库、last_plan.json、发给 DeepSeek
    #      的世界状态，三处都还是相机给的原值，一分都没被这 5mm 污染。
    #    ★ 判据是**来源**不是"第几个动作": 这块方块被抓过之后，
    #      refresh_world_state 会把它那条整条重写成机械臂确认过的坐标（没有 src 戳），
    #      于是下一次抓它就走真实值 —— 这正是用户要的「之后就一直按真实的来」。
    #    ★★ 补多少**看它在纸的哪半边**（half）—— 两半的偏法不一样，各补各的:
    #      左半边（P1/P3 那侧）往机械臂右边挪 (0, −5)，右半边（P2/P4 那侧）X 加 5。
    #      具体数列在 FIRST_GRASP_OFFSET_MM 里（键就是 half）。
    #      half 是 color_vision 在纸自己的坐标系里算好、随这条记录一起写进来的；
    #      没有它（少二维码判不出来、或机械臂已经碰过这块）就**不补** ——
    #      判据缺了时保守不动，比赌一个方向强。
    ...
    if (isinstance(entry, dict)
            and entry.get(WORLD_SRC_KEY) == WORLD_SRC_CAMERA):
        half = entry.get(WORLD_HALF_KEY)
        # half 判不出来 / 不认识 → .get() 给 None → 不补（见上面 ★★）。
        grasp_adj = FIRST_GRASP_OFFSET_MM.get(half)
        if grasp_adj is not None:
            grasp_adj_half = half
            gx, gy = gx + grasp_adj[0], gy + grasp_adj[1]

    grasp_z = nozzle_z(table_z, gl, obj_h, net_h, press)
    ...
    return {
        ...
        # 这一抓有没有为「相机刚拍完的第一抓」补偿过（None = 没补，坐标是原值）。
        # ★ grasp_xy 是**补偿后**的、臂实际会去的点；原始值 = grasp_xy - grasp_adj。
        # ★ grasp_adj_half 是**为什么补了这么多**（"left"/"right"）—— 只给 describe_plan
        #   打印用: 不补时它和 grasp_adj 一起是 None，不会出现"有 half 没 adj"。
        "grasp_adj": grasp_adj,
        "grasp_adj_half": grasp_adj_half,
...
def describe_plan(plan: list[dict], table_z: float | None) -> str:
    ...
    for i, st in enumerate(plan, 1):
        ...
        # ★ 补偿过就得写出来: 上面那行是**臂实际会去的点**，和相机给的对不上
        #   （差 5mm），不说明白就成了"计划里的数字和识别结果对不上"。
        #   没补偿的两种情况都不吭声（half 判不出来 / 机械臂碰过了）—— 都是"就该这样"。
        adj = st.get("grasp_adj")
        if adj:
            raw = (st["grasp_xy"][0] - adj[0], st["grasp_xy"][1] - adj[1])
            # ★ 两半的补法不一样，理由也得跟着换 —— 写死"左半边"的话，右半边那条
            #   记录会把 (+5, 0) 说成"往右挪"，看日志的人照着查就查错了方向。
            if st.get("grasp_adj_half") == WORLD_HALF_RIGHT:
                side, why = "纸的右半边", "吸盘的 X 比实测值大 5mm"
            else:
                side, why = "纸的左半边", "第一抓总偏左"
            lines.append(f"      ★ 相机刚拍的、机械臂还没碰过这块，而且它在**{side}**"
                         f" —— 吸盘点先挪 ({adj[0]:+.1f}, {adj[1]:+.1f})mm"
                         f"（补偿「{why}」）: "
                         f"相机给 ({raw[0]:.2f}, {raw[1]:.2f}) → 实际去 "
                         f"({st['grasp_xy'][0]:.2f}, {st['grasp_xy'][1]:.2f})")
            lines.append(f"        记忆库里仍是相机那个值 —— 这块方块之后一律按"
                         f"真实位置算，和这 {math.hypot(*adj):.0f}mm 无关")
...
def selftest() -> int:
    ...
    # 5m. ★ 「相机刚拍完、机械臂还没碰过这块方块」的第一次抓取: 吸盘点按它在
    #     **纸的哪半边**各挪各的（左 P1/P3 → Y−5；右 P2/P4 → X+5）；其余情况一律
    #     按真实值走。
    #     ★ 四个方向都要钉住，缺一个都会静默走偏:
    #       · 左半边该挪却没挪 → 这条改动白写；
    #       · 右半边该挪 X 却挪了 Y（或没挪）→ 右半边白补，还多带一个方向的误差；
    #       · 判不出半侧（少二维码）却挪了 → 赌错方向；
    #       · 抓过之后还挪 → 5mm 变成每一抓都带的系统性偏差，比不补更糟。
    def t_first_grasp_offset():
        # ★ 两半各补各的（右半边是 2026-09-18 用户后补的要求）。先钉住"表里刚好就
        #   这两半": 将来谁想把"判不出来"也塞进这个表当默认值，这里会先炸。
        assert set(FIRST_GRASP_OFFSET_MM) == {WORLD_HALF_LEFT, WORLD_HALF_RIGHT}, \
            f"补偿表只该有左右两半: {sorted(FIRST_GRASP_OFFSET_MM)}"
        dx, dy = FIRST_GRASP_OFFSET_MM[WORLD_HALF_LEFT]
        rdx, rdy = FIRST_GRASP_OFFSET_MM[WORLD_HALF_RIGHT]
        act = {"obj": "red", "grasp": {"x": 210, "y": 150, "z_level": 0},
               "place": {"x": 260, "y": 120, "z_level": 0}}
        act_before = copy.deepcopy(act)

        def camera_fresh(half=WORLD_HALF_LEFT):
            """相机刚拍的一条记录。half=None 表示判不出半侧（不写那个键）。"""
            e = {"x": 210.0, "y": 150.0, "z_level": 0,
                 WORLD_SRC_KEY: WORLD_SRC_CAMERA}
            if half is not None:
                e[WORLD_HALF_KEY] = half
            return {"red": e}

        # (a) 相机刚拍 + 左半边（P1/P3 那侧）→ 挪。放置点/层数/各个高度一律不许跟着变。
        cam = camera_fresh(WORLD_HALF_LEFT)
        cam_before = copy.deepcopy(cam)
        st = plan_action(act, T, OBJ_H, NET_H, 0.0, DEFAULT_HOVER, cam)
        assert st["grasp_adj"] == FIRST_GRASP_OFFSET_MM[WORLD_HALF_LEFT], st["grasp_adj"]
        assert st["grasp_adj_half"] == WORLD_HALF_LEFT, st["grasp_adj_half"]
        assert st["grasp_xy"] == (210.0 + dx, 150.0 + dy), st["grasp_xy"]
        assert st["place_xy"] == (260.0, 120.0), "放置点不该被这 5mm 碰到"
        plain = plan_action(act, T, OBJ_H, NET_H, 0.0, DEFAULT_HOVER, {})
        assert (st["grasp_z"], st["place_z"], st["grasp_hover"], st["travel_z"]) \
            == (plain["grasp_z"], plain["place_z"], plain["grasp_hover"],
                plain["travel_z"]), "挪的只是 XY，高度一个都不该动"
        # ★ 「不影响后面对该物体实际位置的计算」= 传进来的两份数据一个字都没被改
        assert cam == cam_before, f"记忆库被就地改脏了: {cam}"
        assert act == act_before, f"动作（含模型给的原始坐标）被改脏了: {act}"

        # (a2) ★★ 2026-09-18 用户要求: **右半边（P2/P4 那侧）**的第一抓也要补 ——
        #      补法是「吸盘的 X 比实测值大 5mm」⇒ dx=+5、dy=0。这两条直线钉死，
        #      免得将来谁"顺手把两半写成同一个方向"。
        assert (dx, dy) == (0.0, -5.0), f"左半边该是「往右挪 5」= (0, −5): {(dx, dy)}"
        assert (rdx, rdy) == (5.0, 0.0), f"右半边该是「X 加 5」= (+5, 0): {(rdx, rdy)}"
        right = camera_fresh(WORLD_HALF_RIGHT)
        st_r = plan_action(act, T, OBJ_H, NET_H, 0.0, DEFAULT_HOVER, right)
        assert st_r["grasp_adj"] == FIRST_GRASP_OFFSET_MM[WORLD_HALF_RIGHT], st_r["grasp_adj"]
        assert st_r["grasp_adj_half"] == WORLD_HALF_RIGHT, st_r["grasp_adj_half"]
        assert st_r["grasp_xy"] == (210.0 + rdx, 150.0 + rdy), st_r["grasp_xy"]
        assert st_r["place_xy"] == (260.0, 120.0), "放置点不该被这 5mm 碰到"
        # 挪的只是 XY —— 右半边和左半边一样，高度推导不许跟着变
        assert (st_r["grasp_z"], st_r["travel_z"]) == (st["grasp_z"], st["travel_z"]), \
            "两半的高度推导都只该受挪过的 XY 影响，不该出现第三种高度"

        # (a3) 判不出半侧（少二维码 → 没有这个键）→ 也不许挪。判据缺了要保守，
        #      不能"默认当左边" —— 那等于把一个赌注藏在默认值里。
        unknown = camera_fresh(None)
        st_u = plan_action(act, T, OBJ_H, NET_H, 0.0, DEFAULT_HOVER, unknown)
        assert st_u["grasp_adj"] is None and st_u["grasp_adj_half"] is None \
            and st_u["grasp_xy"] == (210.0, 150.0), \
            f"半侧判不出来时不该补: {st_u['grasp_xy']}"

        # (b) 机械臂确认过的坐标（没有戳）→ 一点都不许挪
        taught = {"red": {"x": 210.0, "y": 150.0, "z_level": 0}}
        st2 = plan_action(act, T, OBJ_H, NET_H, 0.0, DEFAULT_HOVER, taught)
        assert st2["grasp_adj"] is None and st2["grasp_adj_half"] is None \
            and st2["grasp_xy"] == (210.0, 150.0), st2

        # (c) 来源戳的值不是 "camera"（别的来源）→ 也不许挪。判据是「等于相机那个值」，
        #     不是「有这个键」—— 后者会让将来任何一种新来源都白捡 5mm。
        weird = {"red": {"x": 210.0, "y": 150.0, "z_level": 0,
                         WORLD_SRC_KEY: "robot", WORLD_HALF_KEY: WORLD_HALF_LEFT}}
        assert plan_action(act, T, OBJ_H, NET_H, 0.0, DEFAULT_HOVER,
                           weird)["grasp_adj"] is None

        # (d) 计划说明里得能看出「相机给的是哪个、实际去哪个」—— 不写出来就成了
        #     "计划里的数字和识别结果对不上"
        text = describe_plan([st], T)
        assert "相机给 (210.00, 150.00)" in text and "实际去" in text, \
            f"没讲清这一抓挪了 5mm:\n{text}"
        assert "纸的左半边" in text and "纸的右半边" not in text, text
        # ★ 右半边那条也得写清**哪半边、补的哪个方向** —— 复述成"左半边/往右挪"
        #   会让人照着去查错方向（(rdx, rdy) 明明是 (+5, 0)，不是"往右"）。
        text_r = describe_plan([st_r], T)
        assert "相机给 (210.00, 150.00)" in text_r and "实际去 (215.00, 150.00)" in text_r, \
            f"右半边这条没讲清去哪了:\n{text_r}"
        assert "纸的右半边" in text_r and "纸的左半边" not in text_r, text_r
        for other, why in ((st_u, "半侧判不出来"), (st2, "机械臂碰过了")):
            assert "相机给" not in describe_plan([other], T), \
                f"{why}：没挪的时候不该冒出补偿说明"

        # (e) ★★ 「之后就一直按真实的来」: 抓过之后 refresh_world_state 把那**整条**
        #     重写成机械臂确认的坐标（新记录两个戳都没有）→ 再抓它不再挪。
        #     这条同时钉死「5mm 不会一次一次叠加」。
        ws = copy.deepcopy(cam)
        refresh_world_state(ws, [st], [0])
        assert WORLD_SRC_KEY not in ws["red"], f"抓过之后来源戳必须消失: {ws['red']}"
        assert WORLD_HALF_KEY not in ws["red"], f"抓过之后半侧戳必须消失: {ws['red']}"
        assert ws["red"] == {"x": 260.0, "y": 120.0, "z_level": 0}, ws["red"]
        act2 = {"obj": "red", "grasp": {"x": 260, "y": 120, "z_level": 0},
                "place": {"x": 200, "y": 150, "z_level": 0}}
        st4 = plan_action(act2, T, OBJ_H, NET_H, 0.0, DEFAULT_HOVER, ws)
        assert st4["grasp_adj"] is None and st4["grasp_adj_half"] is None \
            and st4["grasp_xy"] == (260.0, 120.0), \
            f"第二次抓同一块方块不该再挪: {st4['grasp_xy']}"
    ck("第一抓补偿按纸的左/右半边各补各的（左 Y−5、右 X+5）；"
       "判不出半侧 / 已抓过都按真实值",
       t_first_grasp_offset)
